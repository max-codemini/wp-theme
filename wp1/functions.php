...
function register_my_menus() { 
	...
	//'「メニューの位置」の識別子' => 'メニューの説明の文字列',
		'sp-nav' => 'SPメニュー',
		'pc-nav' => 'PCメニュー',
		'sp-footer'  => 'SPフッターメニュー',	
		'pc-footer'  => 'PCフッターメニュー',	
	) );
	}
...
function my_error_message($error, $key, $rule){
	...
	if($key === 'name-reading' && $rule === 'noempty'){
        return '※必要事項を入力してください';
    }
	if($key === 'mail' && $rule === 'noempty'){
        return '※必要事項を入力してください';
    }
	if($key === 'textarea' && $rule === 'noempty'){
        return '※必要事項を入力してください';
    }
    return $error;
}
add_filter('mwform_error_message_mw-wp-form-167', 'my_error_message', 10, 3);
